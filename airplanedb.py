...
#==============================================================================
    def get_flight_for_a_day(self, datestring):
        ...
        get_flight_query = ("""select * from FLIGHT where STR_TO_DATE(F_DEPARTURETIME, '%m-%d-%Y:%H:%i') >= STR_TO_DATE('{0}', '%m-%d-%Y') and STR_TO_DATE(F_DEPARTURETIME, '%m-%d-%Y:%H:%i') <= STR_TO_DATE('{1}', '%m-%d-%Y')""").format(datestring, end_date_string)
        cursor = db.cursor()
        try:
            dataList = []
            cursor.execute(get_flight_query)
